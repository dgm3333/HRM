...
def compile_cpp(
    ...
    *,
    ...
    rpaths:
        Runtime library search paths encoded via ``-Wl,-rpath``. When omitted
        for dynamic builds, ``$ORIGIN`` is injected so binaries can locate
        sibling libraries after relocation.
    static:
    ...
    rpath_list = list(rpaths) if rpaths is not None else None
...
def compile_shared_library(
    ...
    *,
    ...
    optional ``ccache`` usage to accelerate iterative builds.
    ``include_dirs``, ``lib_dirs``, ``libs`` and ``rpaths`` mirror the
    parameters of :func:`compile_cpp` allowing shared objects with external
    dependencies to be built in a single step. When ``rpaths`` is omitted
    ``$ORIGIN`` is inserted so the library can resolve sibling dependencies
    after relocation.
    """

    rpath_list = list(rpaths) if rpaths is not None else [Path("$ORIGIN")]
